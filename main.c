...
/*   Updated: 2025/04/22 15:54:25 by idiaz-ca         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(void)
{
	...
	//! mostrar solo la parte que queramos de un string
	printf("%s\n", ft_substr("Hola me llamo Irene", 2, 5));
	//! unir dos cadenas
	printf("%s\n", ft_strjoin("Hola", "Adios"));
	//! cortar lo scaracteres qu ele digamos tanto por delante como por detras del  string
	printf("%s\n", ft_strtrim("supercalifragi", "sui"));
	//! convertir de numero a string
	printf("%s\n", ft_itoa(-14));
	//! strmapi(le pasamos una string al cual le aplicamos una funcion)
	char test(unsigned int i, char c)
	{
		return (c + i); // Por ejemplo, suma el índice al carácter
	}
	int main(void)
	{
		s = "Hola";
		res = ft_strmapi(s, test);
		printf("%s\n", res); // Salida: "Hpmc" (H+0, o+1, l+2, a+3)
		free(res);
		return (0);
	}
	//! striteri(le pasamos un string el cual lo convierte segun la funcion)
	...
	ft_putchar_fd('h', 1);
	ft_putstr_fd("hola", 1);
	ft_putendl_fd("hola me llamo Irene", 1);
	...
	#include "libft.h"
#include <stdlib.h>
// Cuenta cuántas palabras hay en la string separadas por el carácter 'c'
static int	count_words(const char *s, char c)
{
	...
	while (s[i])
	{
		// Si encontramos una letra y no estamos en una palabra,
			empieza una nueva
		if (s[i] != c && in_word == 0)
		{
			in_word = 1;   // Marcamos que estamos dentro de una palabra
			count++;       // Aumentamos el contador de palabras
		}
		// Si encontramos el carácter separador, salimos de la palabra
		else if (s[i] == c)
			in_word = 0;
		i++;
	}
...
// Libera toda la memoria del array de strings en caso de error
static void	free_all(char **result, int i)
{
	while (i--)
		free(result[i]); // Liberamos cada string individual
	free(result);       // Luego liberamos el array completo
}

// Llena el array result con las palabras separadas de 's'
static int	fill_result(char **result, const char *s, char c)
{
	...
	while (s[i])
	{
		// Saltamos todos los caracteres separadores
		while (s[i] == c)
			i++;
		start = i; // Guardamos el inicio de la palabra
		// Avanzamos hasta el final de la palabra
		while (s[i] && s[i] != c)
			i++;
		// Si encontramos una palabra válida (i > start)
		if (i > start)
		{
			// Copiamos la palabra usando ft_substr
			result[j] = ft_substr(s, start, i - start);
			if (!result[j++]) // Si falla malloc en ft_substr
			{
				free_all(result, j); // Liberamos todo lo anterior
				return (0);          // Indicamos error
			}
		}
	}
	result[j] = NULL; // Terminamos el array con un puntero NULL
	return (1);       // Éxito
}

// Función principal que divide la string 's' usando el separador 'c'
char	**ft_split(char const *s, char c)
{
	char	**result;

	if (!s)
		return (NULL); // Si la string es nula, devolvemos NULL
	// Reservamos memoria para el array de strings + 1 para el NULL final
	result = malloc(sizeof(char *) * (count_words(s, c) + 1));
	if (!result)
		return (NULL); // Si falla malloc, devolvemos NULL
	// Llenamos el array con las palabras separadas
	if (!fill_result(result, s, c))
		return (NULL); // Si ocurre un error, devolvemos NULL
	return (result); // Devolvemos el array de strings
}
*/
}